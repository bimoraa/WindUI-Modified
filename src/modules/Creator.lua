...
function Creator.DisconnectAll()
	-- Disconnect every signal without mutating the array mid-iteration.
	-- The old `table.remove(Creator.Signals, idx)` inside a `next` loop shifted indices and
	-- skipped every other entry, leaving roughly half of all connections live even on full
	-- teardown — which compounded every per-element connection leak.
...
function Creator.AddFontObject(Object)
	...
	-- Apply the font to ONLY the newly registered object.
	-- Previously this called Creator.UpdateFont, which re-iterates EVERY font object in the
	-- whole UI on each insertion → O(N²) FontFace allocations while building large UIs/dropdowns.
	-- Creator.UpdateFont is still used for actual runtime font changes (Creator.UpdateFont/SetFont).
