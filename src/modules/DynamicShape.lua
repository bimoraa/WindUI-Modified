...
function DynamicShapeModule:New(Radius, Type, Properties, Children, IsButton, IsSlice)
	...
	Creator.AddSignal(ImageLabel:GetPropertyChangedSignal("AbsoluteSize"), function()
		...
		if string.find(Wrapper.Type, "Squircle") then
			...
			-- Only reassign Image/SliceCenter when the resolved shape actually changes.
			-- Without this guard, every AbsoluteSize change (e.g. each frame of an open/resize
			-- animation, for every Squircle element) re-set ImageLabel.Image to the same asset,
			-- triggering redundant texture work per item per frame.
			if newType ~= Wrapper.Type then
