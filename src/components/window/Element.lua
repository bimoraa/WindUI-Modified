...
return function(Config)
	...
	if Element.Hover then
		...
		-- Previously a NEW MouseMoved connection was created on every MouseEnter and never
		-- removed, so live listeners accumulated without bound — after many hovers, a single
		-- mouse move fired dozens of stale gradient handlers, hurting input latency and FPS.
		local moveConn
		Creator.AddSignal(Main.MouseEnter, function()
			if CanHover then
				...
				moveConn = Creator.AddSignal(Main.MouseMoved, function(x, y)
